...
* **서버-클라이언트 간 멱등성(Idempotency) 체인 구축:** 모바일 기기 네트워크 순단이나 통신 지연 시 동일 예약이 중복 제출되어 대기열 데이터가 중복 생성되는 문제를 차단하기 위해, API 요청 전에 클라이언트 단에서 고유 멱등 키(`waiting_id`)와 등록 시간 원본을 선제적으로 생성·전달하는 정합성 구조를 설계했습니다.
